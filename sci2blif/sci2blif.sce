...
//***********************************
vpr_path='$FPAAHOME/rasp30/vtr_release/vtr_flow/'
...
exec("$FPAAHOME/rasp30/sci2blif/io_info/io_info_rasp30.sce",-1);
exec("$FPAAHOME/rasp30/sci2blif/io_info/io_info_rasp30a.sce",-1);
...
file_list=listfiles("$FPAAHOME/rasp30/sci2blif/sci2pads_added_blocks/*.sce");
...
file_list=listfiles("$FPAAHOME/rasp30/sci2blif/sci2blif_added_blocks/*.sce");
size_file_list=size(file_list);
if file_list ~= [] then
    for bl=1:length(blk_objs)
        for i=1:size_file_list(1)
...
if(makeblk) then 
    ...
    if plcvpr then
        if addvmm then unix_s('$FPAAHOME/rasp30/vtr_release/vpr/vpr $FPAAHOME/rasp30/vpr2swcs/./arch/'+arch+'_arch.xml ' + path + fname + '  -route_chan_width 17 -timing_analysis off -fix_pins ' + path + fname + '.pads -nodisp');
        else unix_s('$FPAAHOME/rasp30/vtr_release/vpr/vpr $FPAAHOME/rasp30/vpr2swcs/./arch/'+arch+'_arch.xml ' + path + fname + '  -route_chan_width 17 -timing_analysis off -fix_pins ' + path + fname + '.pads -nodisp');
        end
        ...
        // generate switches
        unix_s('python $FPAAHOME/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path + ' -route' + brdtype);
    else
        unix_s('python $FPAAHOME/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path + brdtype);
    end
