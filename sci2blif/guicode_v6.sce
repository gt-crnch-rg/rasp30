...
exec('$FPAAHOME/rasp30/sci2blif/fpaacal_fcn.sce',-1);
exec('$FPAAHOME/rasp30/sci2blif/macroblif_gen_fcn.sce',-1);
exec('$FPAAHOME/rasp30/sci2blif/macrocab_gen_fcn.sce',-1);
exec('$FPAAHOME/rasp30/sci2blif/level1_sim_gen_fcn.sce',-1);
exec('$FPAAHOME/rasp30/sci2blif/block_doc_gen_fcn.sce',-1);
exec('$FPAAHOME/rasp30/sci2blif/send_email_ip_pw_fcn.sce',-1);
exec('$FPAAHOME/rasp30/sci2blif/dsgnmtrcs_fcn.sce',-1);
//exec("$FPAAHOME/rasp30/sci2blif/dsgnmtrcs.sce",-1);
exec('$FPAAHOME/rasp30/sci2blif/bcd_fcn.sce',-1);
exec("$FPAAHOME/rasp30/sci2blif/demo_examples/example_inputs.sce");
exec('$FPAAHOME/rasp30/prog_assembly/libs/scilab_code/sftreg_miteADC.sce', -1);
...
file49_menu = uimenu("Parent", file19_menu, "Label", gettext("Passive Channel"), "callback", "demo_example_callback(""pc_1"",""on"",""on"")");
...
file41_menu = uimenu("Parent", file19_menu, "Label", gettext("Ax=b"), "callback", "demo_example_callback(""matmul_sft"",""on"",""onsp"")");
...
file27_menu = uimenu("Parent", file23_menu, "Label", gettext("HHneuron"), "callback", "demo_example_callback(""hhn_offchip"",""on"",""off"")"); 
file28_menu = uimenu("Parent", file23_menu, "Label", gettext("Synfire"), "callback", "demo_example_callback(""sch"",""on"",""off"")"); 
file29_menu = uimenu("Parent", file23_menu, "Label", gettext("Mead SOS"), "callback", "demo_example_callback(""MSOS02_test01"",""on"",""off"")");
...
function Compile_Design_callback(handles)
    ...
    if(extension == '.xcos') then
        ...
        clc;
        if show_dsgnmatrics == 0 then pass_num=1; cap_info = tlist('capinfo'); exec('$FPAAHOME/rasp30/sci2blif/sci2blif.sce', -1); end
        if show_dsgnmatrics == 1 then
            exec("$FPAAHOME/rasp30/sci2blif/dsgnmtrcs.sce",-1);
            pass_num=1; cap_info = tlist('capinfo'); exec('$FPAAHOME/rasp30/sci2blif/sci2blif.sce', -1);
            pass_num=2; exec('$FPAAHOME/rasp30/sci2blif/sci2blif.sce', -1);
            Board_settings();
            Design_Metrics();
        end
        filebrowser();
...
function Program_Design_callback(handles)
    ...
    if(x ~= []) then 
        ...
        if amm_con1 then
            ...
            exec('$FPAAHOME/rasp30/prog_assembly/libs/scilab_code/tunnel_revtun_ver00_gui.sce', -1);
            ...
            tic(); realtimeinit(1); realtime(0); realtime(2);
            exec('$FPAAHOME/rasp30/prog_assembly/libs/scilab_code/switch_program_ver05_gui.sce', -1);
            time1=toc(); string_prog_swc='Program Switches ('+string(time1)+'/'+string(swc_prog_time)+'s)';
            disp('switch_program done');
            progressionbar(winH,[string_tun_revtun string_prog_swc '-> '+string_prog_tar string_dc_setup]);
            tic(); realtimeinit(1); realtime(0); realtime(2);
            exec('$FPAAHOME/rasp30/prog_assembly/libs/scilab_code/target_program_ver02_gui.sce', -1);
            ...
            if (b1==0) then
                ...
                exec('$FPAAHOME/rasp30/prog_assembly/libs/scilab_code/tunnel_clb_ver00_gui.sce', -1);
                disp('tunnel , reverse tunnel for clb done');
                exec('$FPAAHOME/rasp30/prog_assembly/libs/scilab_code/switch_program_ble_ver00_gui.sce', -1);
...
function View_Routing_callback(handles)
    ...
    if(x ~= []) then 
        ...
        if(x ~= []) then //pads file is in hidden dir
            ...
            //if plcvpr & addvmm then unix_s('python $FPAAHOME/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path + ' -vmm -route' + brdtype +' -v');
            if plcvpr & addvmm then unix_s('python $FPAAHOME/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path + ' -route' + brdtype +' -v');
            elseif plcvpr then unix_s('python $FPAAHOME/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path+ ' -route' + brdtype+' -v');
            //elseif addvmm then unix_s('python $FPAAHOME/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path + ' -vmm' + brdtype+' -v');
            elseif addvmm then unix_s('python $FPAAHOME/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path + brdtype+' -v');
            else unix_s('python $FPAAHOME/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path + brdtype+' -v');
            end
...
function Take_Data_callback(handles)
    ...
    else exec('$FPAAHOME/rasp30/prog_assembly/libs/scilab_code/voltage_measurement_gui.sce', -1);
    end    
    disp("Data Collected ");
    filebrowser();
endfunction
...
function Send_Email_callback(handles)
    ...
    if (email_name == []) then
        ...
        exec('$FPAAHOME/rasp30/sci2blif/send_email_ip_pw.sce',-1);
    end
endfunction
...
function Load_Data_callback(handles)
    ...
    disp("Loading Data...");
    unix_s('python $FPAAHOME/rasp30/sci2blif/rmunzip.py ' + path)
    exec('$FPAAHOME/rasp30/prog_assembly/libs/scilab_code/outhexfile2voltfile_gui.sce', -1);
    ...
    if amm_con1 then 
        ...
        if(extension == '.blif') then 
            ...
            if(x ~= []) then //pads file is in hidden dir
                ...
                unix_s('python $FPAAHOME/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path);
                unix_s('mv ' + fname' + '.pads ' + fname + '.place ' + fname + '.net ' + fname + '.route ' +hid_dir);
...
function Reset_Com_callback(handles)
    ...
    unix_s('$FPAAHOME/rasp30/sci2blif/usbreset');
    disp('Done!');
endfunction

function demo_example_callback(demo_fname,com_prep_index,arb_msg_index)
    global file_name path fname extension;
    unix_g("mkdir $FPAAHOME/RASP_Workspace/demo_examples/");
    unix_g("mkdir $FPAAHOME/RASP_Workspace/demo_examples/"+demo_fname);
    unix_s("cp $FPAAHOME/rasp30/sci2blif/demo_examples/"+demo_fname+".xcos $FPAAHOME/RASP_Workspace/demo_examples/"+demo_fname+"/");
    file_name= '$FPAAHOME/RASP_Workspace/demo_examples/"+demo_fname+"/"+demo_fname+".xcos'; disp(demo_fname); disp(file_name);
    ...
    if arb_msg_index == "onsp" then arb_msgsp(); end
endfunction
...
function open_document(doc_name)
    [a,b]=unix_g("acroread $FPAAHOME/rasp30/sci2blif/documentation/"+doc_name+" &"); 
...
function doc6_callback()
    ...
    disp('Step 2 of 3');
    unix_g('cd $FPAAHOME/rasp30/sci2blif/documentation && sudo gdebi -n AdbeRdr9.5.5-1_i386linux_enu.deb');
    disp('Step 3 of 3');
    unix_s('cd '+path); 
...
function gui1_callback()
    exec('$FPAAHOME/rasp30/sci2blif/fpaacal.sce',-1);
endfunction

function gui4_callback()
    exec("$FPAAHOME/rasp30/sci2blif/macroblif_gen_gui.sce");
endfunction

function gui6_callback()
    exec("$FPAAHOME/rasp30/sci2blif/macrocab_gen_gui.sce");
endfunction

function gui9_callback()
    exec("$FPAAHOME/rasp30/sci2blif/bcd_gui.sce");
endfunction

function gui8_callback()
    exec("$FPAAHOME/rasp30/sci2blif/level1_sim_gen_gui.sce");
endfunction

function gui2_callback()
    exec("$FPAAHOME/rasp30/sci2blif/block_doc_gen_gui.sce");
endfunction
...
csvpath="$FPAAHOME/RASP_Workspace";
...
function etc10_callback(handles)
    ...
    csvdir=uigetdir("$FPAAHOME/RASP_Workspace", "Choose a directory.")
...
function etc14_callback(handles)
    ...
    path="$FPAAHOME/RASP_Workspace/compilation_check/";
    mkdir(path);
    cd(path);
    ...
    unix_w("cp $FPAAHOME/rasp30/sci2blif/xcos_ref/compilation_check/compile_check_30.xcos "++fname+extension);
    pass_num=1; cap_info = tlist('capinfo'); exec('$FPAAHOME/rasp30/sci2blif/sci2blif.sce', -1);
    ...
    unix_w("cp $FPAAHOME/rasp30/sci2blif/xcos_ref/compilation_check/compile_check_30_vmm.xcos "++fname+extension);
    pass_num=1; cap_info = tlist('capinfo'); exec('$FPAAHOME/rasp30/sci2blif/sci2blif.sce', -1);
...
function etc15_callback(handles)
    global file_name path fname extension board_num chip_num;
    clc
    exec("$FPAAHOME/rasp30/sci2blif/convert2modelica.sce",-1);
...
function arb_msgsp()
    messagebox("After the take data step, run sftreg_miteADC(output_number) to view the corresponding output 1-4");
endfunction
function com_prep()
    clear scs_m;
    clear blk_objs;
...
function doc_block_callback()
    global fname;
    [a,b]=unix_g('firefox $FPAAHOME/rasp30/sci2blif/documentation/block_docs/build/html/'+fname+'.html'); 
    if b == 1 then
...
function doc_full_callback()
    [a,b]=unix_g('firefox $FPAAHOME/rasp30/sci2blif/documentation/block_docs/build/html/index.html'); 
    if b == 1 then
        messagebox("Error opening documentation. ", "Unable to open firefox! ", "scilab")
    end
endfunction
